...
** Last update Sun Nov 15 14:06:04 2015 berdrigue bongolo-beto
*/

#include <stdarg.h>
#include "./include/my_printf.h"

void	init_flags(char flag, int *flag_cp, t_pf_format *ptf_format, int *v_set)
{
  if (*v_set == 0)
    {
  ...
  if (my_get_char_pos("-+ 0#", flag) != -1 && *flag_cp < 3)
    {
...
void	init_precision(char *format, int *i, int *var_set, t_pf_format *pt_form)
{
  if (format[*i - 1] == '.' && format[*i] != '*')
    {
      pt_form->precision = get_str_nbr(format, i);
    }
  else if (format[*i - 1] == '.' && format[*i] == '*')
    {
      pt_form->precision = my_strdup("*");
    }
}
  ...
  while (format[*i])
    {
      ...
      if (my_get_char_pos("hl", format[*i]) != -1)
	ptf_format->length_modifier = format[*i];
      else if (is_alpha(format[*i], 0) || format[*i] == '%')
	{
	  if (format[*i] == 'n')
	    ptf_format->field_width = my_strdup(my_itoa(*cp));
