...
** Last update Sat Nov 28 10:29:58 2015 berdrigue bongolo-beto
*/
...
{
  if (my_get_char_pos(ptf_f->flags, '0') != -1 &&
      ptf_f->precision == 0)
    {
      *len_p = *width;
      *width = 0;
...
int	nbr_precision_handler(int width, int len_p, int nbr, t_pf_format *ptf_f)
{
  ...
  cp = 0;
  if (my_get_char_pos(ptf_f->flags, '+') != -1 && nbr < 0 || nbr < 0)
    {
      my_putchar('-');
      nbr = nbr * (-1);
      width = width - 1;
  ...
  else if (my_get_char_pos(ptf_f->flags, '+') != -1 && nbr > 0)
    my_putchar('+');
...
int		pnbr_sd(t_pf_format *ptf_format, int *nbr, int width, int len_p)
{
  ...
  if (my_get_char_pos(ptf_format->flags, '+') != -1 || *nbr < 0)
    {
      width = width - 1;
      cp = cp + 1;
    }
  ...
  else if ((my_get_char_pos(ptf_format->flags, '+') != -1 && *nbr > 0))
    my_putchar('+');
  nbr_space_handler(ptf_format, nbr, &cp);
  cp = cp + print_nchar('0', len_p);
  (nbr2 == 0) ? my_put_nbr(*nbr) : my_putnbr(nbr2);
  return (cp);
}
...
int	print_nbr(va_list ap, t_pf_format *ptf_format)
{
  ...
  ((!res_set_prec || len_precision < my_nbr_len(nbr)) ? len_precision = 0 : 0);
  cp = my_nbr_len(nbr);
  width = (width > my_nbr_len(nbr) ? width : 0);
